...
module.exports.generateData = function(cards) {
	...
	var regex;
	if (settings.titleReplacePattern)
		regex = new RegExp(settings.titleReplacePattern);

	cards.forEach(function(item) {
		var card = {
			"name": regex ? item.name.replace(regex, "", "gi") : item.name,
			"labels": getLabels(item),
			"link": settings.exportLinks ? item.url : null,
			"shortLink": item.shortUrl,
			"shortHash": item.shortUrl.replace("https://trello.com/c/", ""),
			"idShort": item.idShort,
			"releasenotes": []
		};

		console.log(item.name.replace(regex, ""));
