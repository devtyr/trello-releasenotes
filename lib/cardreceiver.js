...
var CardReceiver = function(applicationKey, userToken) {
...
CardReceiver.prototype.receive = function(lists, callback) {
	...
	me.getLists(function(err, foundLists) {
		...
		if (foundLists.indexOf("invalid id") > -1) {
			console.log("No lists found. This might be due to an invalid board id.");
			return;
		}
		if (err) {
			console.log(err);
...
CardReceiver.prototype.getLists = function(callback) {
	makeRequest(rest.get, this.trello.uri + '/1/boards/' + settings.boardId + '/lists', { query: this.trello.createQuery() }, callback);
}
